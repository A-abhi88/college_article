...
if ($conn->connect_error) {
  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "DELETE FROM all_article WHERE id='$id'";
...
<body>
  ...
  <?php
        if($d==1){
          echo"<p style='color:green;'>Article delete successfuly</p>";
          
        }
      $sql = "SELECT * FROM all_article";
      $result=mysqli_query($conn,$sql);
  ...
  echo'
    <div class="article">
      <h1 >'.$row["title"].'</h1> 
            ...
            <form style="margin:10px 0px;" method="POST" action="edit_articles.php"><input style="display:none;" type="text" name="id" value='.$row["id"].'> <button class="deleteBtn">Delete</button></form>  
    </div> ';
}
}else{
